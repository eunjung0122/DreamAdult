...
import test.file.dto.FileCommentDto;
import test.qna.dto.QnACommentDto;
import test.util.DbcpBean;

public class FileCommentDao {
	private static FileCommentDao dao;
	private FileCommentDao() {}
	public static FileCommentDao getInstance() {
		if(dao==null) {
			dao=new FileCommentDao();
		}
		return dao;
	}
	public List<FileCommentDto> getMyList(FileCommentDto dto){
		List<FileCommentDto> list=new ArrayList<>();
		...
		try {
			...
			while (rs.next()) {
				FileCommentDto dto2=new FileCommentDto();
	...
	public int getMyCount(FileCommentDto dto) {
		int count=0;
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	//댓글 목록을 리턴하는 메소드
	public List<FileCommentDto> getList(FileCommentDto dto2){
		List<FileCommentDto> list = new ArrayList<>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int getSequence() {
		...
		try {
			...
			conn = new DbcpBean().getConn();
			//실행할 sql문 작성
			String sql = "SELECT board_file_comment_seq.NEXTVAL AS seq"
					+ " FROM DUAL";
			//PreparedStatement 객체의 참조값 얻어오기
			pstmt = conn.prepareStatement(sql);
			//?에 바인딩할 내용이 있으면 여기서 바인딩
	...
	//댓글 추가하는 메소드
	public boolean insert(FileCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql문 작성
			String sql = "INSERT INTO board_file_comment"
	...
	public boolean delete(int num) {
		...
		try {
			conn = new DbcpBean().getConn();
			//실행할 sql문 작성
			String sql = "UPDATE board_file_comment"
					+ " SET deleted='yes'"
					+ " WHERE num=?";
			pstmt = conn.prepareStatement(sql);
			// ? 에 바인딩할 내용이 있으면 여기서 바인딩
	...
	//댓글을 삭제하는 메소드
	public boolean update(FileCommentDto dto) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		int flag = 0;
		try {
